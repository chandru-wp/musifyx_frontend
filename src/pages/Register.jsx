...
export default function Register() {
    ...
    const submit = async () => {
        ...
        try {
            ...
            alert("Registration Successful! Welcome to MusifyX.");
            navigate("/login");
        } catch (err) {
            console.error("Full Error Object:", err);
            const msg = err.response?.data?.msg || err.response?.data?.error || "Registration failed. Please check if backend is running on port 5000.";
            alert(msg);
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="h-screen flex items-center justify-center bg-gradient-to-b from-[#1db95433] to-black p-4 font-sans">
            <div className="bg-black p-8 md:p-12 rounded-2xl w-full max-w-md shadow-2xl border border-spotify-gray overflow-y-auto max-h-[95vh] custom-scroll">
